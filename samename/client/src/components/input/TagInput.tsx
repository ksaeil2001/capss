...
const TagInput: React.FC<TagInputProps> = ({
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      if (showSuggestions) {
        const nextActive = activeSuggestion < filteredSuggestions.length - 1 ? activeSuggestion + 1 : 0;
    ...
    if (e.key === 'ArrowUp') {
      e.preventDefault();
      if (showSuggestions) {
        const prevActive = activeSuggestion > 0 ? activeSuggestion - 1 : filteredSuggestions.length - 1;
